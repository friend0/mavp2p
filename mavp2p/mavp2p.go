...
func NewUDPClientEndpoint(address string) (EndpointType, error) {
	return EndpointType{
		"dest_ip:port",
		"udp, client mode",
...
func NewUDPServerEnpoint(address string) (EndpointType, error) {
	return EndpointType{
		"listen_ip:port",
		"udp, server mode",
		func(args string) (gomavlib.EndpointConf, error) {
...
func DefaultConfig() CommandLineArguments {
...
func NewProgram(endpoints []string, config *CommandLineArguments) (*Program, error) {
	cli := DefaultConfig()
	if config != nil {
		cli = *config
	}
	endpointConfs, err := generateEndpointConfs(endpoints)
	if err != nil {
		return nil, err
	}
...
func NewProgramFromCLI(args []string) (*Program, error) {
	...
	for {
		...
		case e := <-p.node.Events():
			...
			case *gomavlib.EventFrame:
				// todo: add config to program struct
				// if cli.Print {
